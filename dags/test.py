...
def test_duckdb():
    ...
    def create_df() -> pd.DataFrame:
        ...
        """
        df = pd.read_csv('dags/ingestion/data/dados_dengue_3529708_1_53_2023_2023.csv')
        return df
    
    # @task
    # def read_csv_aws1(bucket_name, object_name):
    #     try:            
    #         s3 = boto3.client('s3')
    ...
    @task
    def create_duckdb_table_from_pandas_df1(csv_df):
        if csv_df is None:
        ...
        try:
            ...
            conn.execute(
                """CREATE TABLE IF NOT EXISTS raw_tab_dengue AS 
            ...
            # get the table
            teste_dados = conn.execute("SELECT * FROM raw_dengue;").df()
            print(teste_dados.head())
            return teste_dados
        
        except Exception as e:
            logger.error(f"Erro ao criar a tabela no DuckDB: {e}")

    # csv_data = read_csv_aws1(bucket_name, object_name)
    csv_data = create_df()
    create_duckdb_table_from_pandas_df1(csv_data)
